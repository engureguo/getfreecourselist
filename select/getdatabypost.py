...
headers = {
    ...
    'Cookie': 'insert_cookie=xxxxx; selectionBar=1293218; JSESSIONID=xxxx'    
}
...
def keepData(clist):
    ...
    df = pd.DataFrame(d)
    now = time.asctime( time.localtime(time.time()))
    fn = 'courstlist_' + now.replace(' ','-') + '.xlsx'
...
def getWantedData(clist):
    ...
    l = [] # 课程号
    with open('kch.txt', 'r')as f:
        l = f.read().split(',')
    info = []
    for c in clist:
        if c['kch'] in l and int(c['bkskyl']) > 0 :
            d = {}
            d['课程名'] = c['kcm']
            d['课余量'] = c['bkskyl']
            d['课程号'] = c['kch']
